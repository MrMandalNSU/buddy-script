...
export const authService = {
  async login(input: LoginInput): Promise<Result<Session>> { try { const data = await apiRequest<z.infer<typeof authSchema>>("/api/v1/auth/login", { method: "POST", body: { email: input.email.trim(), password: input.password, remember: input.remember }, schema: authSchema, authRetry: false }); return { ok: true, data: { user: user(data.user), accessExpiresAt: data.session.accessExpiresAt } }; } catch (error) { return normalized(error); } },
